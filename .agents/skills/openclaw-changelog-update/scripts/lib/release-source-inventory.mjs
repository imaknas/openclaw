...
function patchRoundTripRestoresTree(cwd, targetCommit, targetTree, patch, applyMode) {
  ...
  try {
    ...
    if (applyPatch(true).status !== 0) {
      return false;
    }
    if (applyPatch(false).status !== 0) {
      return false;
    }
...
export function buildReleaseSourceInventory(
  ...
  if (new Set(explicitProvenanceTargets).size !== explicitProvenanceTargets.length) {
    fail("trusted adapted and partial provenance target commits must be disjoint");
  }
  ...
  for (const record of sourceRecords) {
    ...
    const adaptedDetails = trustedAdaptedPullRequestDetails.get(record.commit);
    if (
